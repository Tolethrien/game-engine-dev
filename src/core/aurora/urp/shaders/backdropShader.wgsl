...
// canvas sized, the gui drawn so far; also the size mip 0 is measured against
...
// level 0 reads the scene (and gui over it), the other levels read the level below
override compose: bool = false;
// the scene pyramid is built before any gui is drawn, it reads the scene alone
override withGui: bool = true;
...
fn fetch(uv: vec2f) -> vec4f {
  if (compose) {
    ...
    // same composition as screenShader.wgsl: both premultiplied and linear
